...
def upload_playlist():
    ...
    try:
        ...
        cursor.execute("SELECT MAX(id) FROM DB_TABLE ")
        ...
        cursor.execute("UPDATE DB_TABLE  SET m3u_url = %s WHERE id = %s", (final_path, temp_playlist_id))
        conn.commit()
        cursor.execute("UPDATE DB_TABLE  SET id = %s WHERE id = %s", (playlist_id, temp_playlist_id))
        conn.commit()
...
def update_playlist():
    ...
    try:
        ...
        cursor.execute("SELECT owner_password_hash FROM DB_TABLE  WHERE id = %s", (playlist_id,))
        ...
        cursor.execute("UPDATE DB_TABLE  SET timestamp = %s WHERE id = %s", (datetime.today().strftime('%d-%m-%Y'), playlist_id))
        conn.commit()
        cursor.close()
        conn.close()
...
def get_playlists():
    ...
    cursor.execute("SELECT id, reddit_user, service_name, countries, main_categories, epg_url, github_epg_url, timestamp, donation_info FROM DB_TABLE  WHERE display = 1")
...
def manual():
    ...
    db = get_connection()
    cursor = db.cursor()
    cursor.execute("SELECT m3u_url FROM DB_TABLE  WHERE id = %s", (id_selected,))
    row = cursor.fetchone()
